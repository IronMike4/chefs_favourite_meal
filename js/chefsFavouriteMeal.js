...
// Prompt user for an ingredient and fetch meals from API
async function takeOrder() {
    ...
    if (data.meals === null) {
        alert("No meals found for the entered ingredient. Please try a different one.");
        // Recursively prompt again
        return takeOrder();  
    }
    ...
    saveOrder(order);
    alert(`Order placed: ${order.description}, Order Number: ${order.orderNumber}`);
    displayOrders();
}
...
function completeOrder() {
    ...
    const orderNumber = prompt(orderList + "Enter the order number to mark as complete (or 0 to cancel):");
    if (orderNumber === '0') {
        displayOrders();
        return;
    }
    ...
    const orderToComplete = orders.find(order => order.orderNumber == orderNumber);
    if (!orderToComplete) {
